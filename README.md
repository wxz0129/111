...
**这是什么？** 这是一个设计的 Python 自动化工具。它能一键扫描你文件夹里乱七八糟的 PDF 研报和 Excel 模型，识别是哪家公司的，然后自动重命名为标准格式（`券商-公司-标题-日期`），并分门别类归档。
...
### 第 2 步：建立工作台
为了让工具正常运行，请确保代码文件（后缀.py）与Investment Research文件夹在同一路径下运行
